# moved from src/pii_scanner/detectors/patterns.py
import re
...
def build_detectors() -> List[Detector]:
	# DEMO: All validators are None (no validation), regexes are more permissive
	return [
		   Detector(
			   name="AADHAAR",
			   pattern=re.compile(r"([2-9][0-9]{3}[\s\-\n\r]*[0-9]{4}[\s\-\n\r]*[0-9]{4})"),
			   validator=None,
		...
		Detector(
		   ...
		   Detector(
			   name="IFSC",
			   pattern=re.compile(r"([A-Z]{4}[0-9A-Z]{7,})"),
		   ...
		   Detector(
			   name="TAN",
			   pattern=re.compile(r"([A-Z]{4}[0-9]{5}[A-Z])"),
